...
#define t1s_dbg(fmt, ...) do { pr_info("microchip_t1s: %s: " fmt, __func__, ##__VA_ARGS__); } while (0)
...
static int lan865x_generate_cfg_offsets(struct phy_device *phydev, s8 offsets[2])
{
	...
	t1s_dbg("generate_cfg_offsets: start\n");
	for (int i = 0; i < ARRAY_SIZE(fixup_regs); i++) {
		t1s_dbg("offset loop %d\n", i);
		ret = lan865x_revb0_indirect_read(phydev, fixup_regs[i]);
		if (ret < 0)
			t1s_dbg("indirect_read failed: %d\n", ret);
			return ret;
		if (ret & BIT(4))
			offsets[i] = ret | 0xE0;
		else
			offsets[i] = ret;
		t1s_dbg("offset[%d]=0x%02x\n", i, offsets[i]);
	}
...
static int lan865x_write_cfg_params(struct phy_device *phydev, u16 cfg_params[])
{
	...
	t1s_dbg("write_cfg_params: start\n");
	for (int i = 0; i < ARRAY_SIZE(lan865x_revb0_fixup_cfg_regs); i++) {
		t1s_dbg("write reg=0x%04x val=0x%04x\n", lan865x_revb0_fixup_cfg_regs[i], cfg_params[i]);
		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
				    lan865x_revb0_fixup_cfg_regs[i],
				    cfg_params[i]);
		if (ret)
			t1s_dbg("phy_write_mmd failed: %d\n", ret);
			return ret;
	}
...
static int lan865x_revb0_config_init(struct phy_device *phydev)
{
	 ...
	 */
	t1s_dbg("revb0_config_init: start\n");
	for (int i = 0; i < ARRAY_SIZE(lan865x_revb0_fixup_registers); i++) {
		t1s_dbg("write reg=0x%04x val=0x%04x\n", lan865x_revb0_fixup_registers[i], lan865x_revb0_fixup_values[i]);
		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
...
static int lan867x_revb1_config_init(struct phy_device *phydev)
{
	 ...
	 */
	t1s_dbg("revb1_config_init: start\n");
	...
	for (int i = 0; i < ARRAY_SIZE(lan867x_revb1_fixup_registers); i++) {
		t1s_dbg("modify reg=0x%04x mask=0x%04x val=0x%04x\n", lan867x_revb1_fixup_registers[i], lan867x_revb1_fixup_masks[i], lan867x_revb1_fixup_values[i]);
		...
		if (err)
			t1s_dbg("phy_modify_mmd failed: %d\n", err);
			return err;
	}
